...
async def refresh_token(Authorize: AuthJWT = Depends()):
    ...
    try:
        access_lifetime = datetime.timedelta(minutes=60)
        Authorize.jwt_refresh_token_required()  # valid access token talab qiladi
        current_user = Authorize.get_jwt_subject()  # access tokendan username ni ajratib oladi

        # Databasedan usernni filter orqali topamiz
        db_user = session.query(User).filter(User.username == current_user).first()
        if db_user is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")

        #  access token yaratmiz
